...
## [2026-07-20] lesson | confluence-daily-status-design
<!-- kilroy-log date=2026-07-20 skill=session-recap event=recap status=ok type=lesson slug=confluence-daily-status-design -->
## [2026-07-20] setup | confluence-daily-status machine setup progress + connector-consolidation plan approved

Confirmed `@playwright/mcp` package name and `--user-data-dir`/`--browser` flags against the real README (`.mcp.json`'s earlier `--channel` guess was already corrected). Found Node.js 24.18.0 already installed via winget and in PATH, ran `mcp-server`'s `npm install`, created `.env` from `.env.example`, and created a dedicated (not-yet-signed-in) Playwright browser profile directory. Separately: Jordan asked whether `kilroy-connectors` is being replaced by Nova's MCPs, which led to a full connector-architecture review and a big scope decision -- retire AMR Hub and Master Tracker CSV entirely (no Nova equivalent exists for either), retire `arriving-amr-progress` outright (its only data source), gut `fleet-commissioning-handoff` to an Overmind-only fleet-state report, and consolidate Planner onto Nova's `planner` MCP as the sole path (retiring `kilroy-connectors`' `planner_get_tasks` tool). Overmind stays on its existing typed `kilroy-connectors` tool. `run-daily-workflow` scopes down rather than retiring; CLAUDE.md's opening identity paragraph and `.claude/hooks/session-start.sh` are both in scope for the same change. Full plan approved and written to `C:\Users\jocasias\.claude\plans\cozy-purring-crown.md` -- execution not yet started (deferred to a later session). Details in [[Knowledge/Lessons/2026-07-20-connector-consolidation-planning]].
<!-- kilroy-log date=2026-07-20 skill=session-recap event=recap status=ok type=setup slug=connector-consolidation-planning -->
## [2026-07-21] lesson | connector-consolidation
<!-- kilroy-log date=2026-07-21 skill=session-recap event=recap status=ok type=lesson slug=connector-consolidation -->
## [2026-07-21] lesson | autonomy-buildout
<!-- kilroy-log date=2026-07-21 skill=session-recap event=recap status=ok type=lesson slug=autonomy-buildout -->
## [2026-07-21] setup | autonomy tickets 01-08 executed, pushed, CI green

Executed the approved connector consolidation (AMR Hub + Master Tracker + Graph-API Planner retired; Overmind sole typed tool; CLAUDE.md rewritten with Jordan's diff approval; /floor-init re-run flag on record). Built the full autonomy scaffolding: four-phase autonomous run-daily-workflow (markers + journal, Mon-Thu, hard no-post rule headless), tier-ruled day checklist + Obsidian vault mirror, Loop dual-path + monitored-chats template, tracked session-start hook (scripts/hooks/), Windows Task Scheduler scripts (scripts/windows/, UNVERIFIED-until-Windows), learning loop (journal.jsonl + Thursday reflection drafts), and docs/bootstrap-windows.md with the committed bootstrap-pending marker. Tickets 03-08 ran as four parallel worktree subagents, merged same session. Pushed to origin; GitHub Actions ran the full mcp-server suite 20/20 green (verifying the server tests the Mac sandbox couldn't). Remaining: ticket 09's work-PC verification week, starting with tomorrow's bootstrap. Details in [[Knowledge/Lessons/2026-07-21-connector-consolidation]] and [[Knowledge/Lessons/2026-07-21-autonomy-buildout]].
<!-- kilroy-log date=2026-07-21 skill=session-recap event=recap status=ok type=setup slug=autonomy-buildout-executed -->
