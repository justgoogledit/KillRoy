...
Five workflows, plus two foundational checks (one gates runtime data pulls, one gates skill edits):
...
- **Skill sprawl.** Five Jordan-facing workflows now (plus `check-connectors` and `verify-fixtures`, which are infrastructure, not workflows) -- `triage-personal-items` and `confluence-daily-status` are both one-off, on-record exceptions to the 3+ repeats rule (see Scope), not a standing alternate trigger. Add a sixth workflow only when Jordan hits the same manual task 3+ times. A future request to bend that rule again needs its own explicit, on-record exception the same way -- it doesn't inherit permission from either of these two.
