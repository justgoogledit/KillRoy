...
outputs:
  ...
  - log.md append, but only when at least one pairing (or the coverage check) fails -- never on an all-green run
---
...
1. Read `Knowledge/Sources/fixtures/README.md` fresh (never a cached or hardcoded copy) at the start of every run. Parse its "Files" table, its "How to dry-run a skill against these fixtures" section (the exact fixture-substitution invocation per skill, including the swap pattern for broken/stale variants and any synthetic env overrides), and its "Deliberate edge cases baked in" section (the specific behavior each edge case must produce) to build this run's list of fixture-skill pairings.
...
4. For each in-scope pairing, dry-run the target skill exactly per the README's documented invocation for that pairing: substitute the named fixture file(s) for the live call(s) they stand in for, and run every one of the target skill's own Steps AND its own `## Verify` section, in full, exactly as written -- including any synthetic env override the README specifies (e.g. `GRAPH_API_USER_OBJECT_ID` for the Planner fixture) and any file-mtime precondition (`touch -d "48 hours ago" Knowledge/Sources/fixtures/master-tracker-stale.csv` immediately before the stale-CSV pairing, per the README's Stale-CSV section).
5. **Deliberate deviation from a full real invocation**: stop each target skill short of its own final "write to `Projects/<x>`" step and its own "append to `log.md`" step. This check exercises the skill's data-handling and Verify logic, not its delivery -- writing real `Projects/` artifacts or real `log.md` lines on every fixture-check run (which fires on every `skill-creator` edit) would sprawl `Projects/` and pollute Jordan's real operational log with synthetic numbers, the same reasoning the 2026-07-17 dry-run session already established by hand -- see [[Knowledge/Lessons/2026-07-17-execution-test-with-fixtures]].
...
8. If every in-scope pairing PASSed (and, on an unscoped run, coverage is complete), end with "All fixtures verified -- safe to consider this skill edit done." If anything FAILed, end with "FIXTURE REGRESSION -- do not consider this skill edit done until fixed" and list which pairing(s) failed and why. Then, only on that FAIL path, append one line to the real `log.md`: `## [<date>] setup | fixture-check failed -- <target-skill>: <fixture>, ...` (one entry per failure). Do not append anything on an all-green run, or on a scoped run against a skill with no documented pairings -- same no-noise rule as `check-connectors`.
...
3. **No delivery side effects**: no file under `Projects/` changed, and no line was appended to the real `log.md` by any target skill driven during this run. Only this skill's own single summary line, and only on failure.
...
**Good trigger:** *"kilroy verify fixtures"* -> unscoped, full matrix (currently 6 pairings across 3 skills) plus the coverage cross-check.
**Good trigger (automatic, scoped):** `skill-creator` finishes editing `arriving-amr-progress` and runs this scoped to `arriving-amr-progress` -> only that skill's 2 pairings run, no coverage cross-check.
