...
inputs: none required beyond MCP access to `confluence`, Nova's `planner` MCP, and the `playwright` MCP server
outputs:
...
- Nova's `planner` MCP (already used by run-daily-workflow's Planner digest) -- reused here for draft content, not re-fetched separately when this step runs after that pull in the same session.
- run-daily-workflow's prior day file's Carry-over section -- reused for draft content, same reuse-not-refetch principle. (The AMR-gate-board context bullet was dropped in the 2026-07-21 consolidation along with its source skill.)
...
4. **Draft the bullets.** Compose from:
   - Today's Planner tasks assigned to Jordan, due today, via Nova's `planner` MCP (reused from run-daily-workflow's own pull when running as its step; a fresh pull if invoked in isolation).
