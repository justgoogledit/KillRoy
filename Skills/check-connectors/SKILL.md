...
- `kilroy-connectors` MCP server (`mcp-server/`, registered in `.mcp.json`) -- the AMR Hub check goes through its `amr_hub_get_units` tool and the Overmind check through `overmind_get_fleet_state` (reachability mode) rather than prose-described HTTP; the server's `node:test` suite is what proves both contracts. Master Tracker and Planner checks stay prose-described until tickets #9-#10 migrate them.
