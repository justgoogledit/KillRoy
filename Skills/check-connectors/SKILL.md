...
- `kilroy-connectors` MCP server (`mcp-server/`, registered in `.mcp.json`) -- the AMR Hub check goes through its `amr_hub_get_units` tool, the Overmind check through `overmind_get_fleet_state` (reachability mode), and the Master Tracker check through `master_tracker_get_rows`; the server's `node:test` suite is what proves all three contracts. The Planner check stays prose-described until ticket #10 migrates it.
